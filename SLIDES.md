...
Syntax: `@inject <js-file> <start-seconds> <duration-seconds> [data-file]`
...
- At the specified time during playback the function is called; it renders into the allocated area.
- **Filenames with spaces** must be quoted: `@inject "my chart.js" 2 5 "sales data.json"`
...
export default function(inFn, outFn, dataFn) {
  ...
  // dataFn is provided when a data file is specified in @inject.
  // dataFn()           → fetch Response for the default data file
  // dataFn("other.bin") → fetch Response for any named file in _inject/
  // Call .text(), .json(), .arrayBuffer(), etc. on the Response.
  if (dataFn) {
    dataFn().then(r => r.json()).then(data => render(data));
  }

  function render(data) {
    const el = document.createElement('div');
    el.style.color = 'white';
    el.textContent = data?.label ?? 'No data';
    outFn(el); // renders the element in the allocated slide area
  }
}
```

**Binary example** (loading an image from `_inject/`):
